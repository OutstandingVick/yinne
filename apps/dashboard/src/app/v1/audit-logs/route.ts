...
import { ApiError, page, paginationQuerySchema } from "@yinne/contracts";
...
export function GET(request: NextRequest) {
  return apiRoute(request, async (context) => {
    ...
    const rows = await listAuditLogs(context, query.limit, after);
    return page(rows, query.limit, (entry) =>
      Buffer.from(entry.createdAt.toISOString()).toString("base64url"),
    );
  });
}
