...
const principalId = (principal: RequestContext["principal"]) =>
  principal.type === "user"
    ? principal.userId
...
export const subscriptionView = (row: typeof subscriptions.$inferSelect) => ({
  ...
  billing_timezone: row.billingTimezone,
  ...
  trial_start: row.trialStart?.toISOString() ?? null,
  trial_end: row.trialEnd?.toISOString() ?? null,
...
export async function createSubscription(
  ...
  return withTenantTransaction(context.tenant, async (tx) => {
    await requirePermission(tx, context.principal, "subscriptions:write", {
    ...
    if (context.tenant.environment === "live" && input.trial_days > 0)
      throw new ApiError(
        409,
        "conflict",
        "recurring_capability_required",
        "Live trials require an approved recurring payment capability.",
      );
    const actor = principalId(context.principal);
    const keyDigest = digest(key);
    const requestDigest = digest(JSON.stringify(input));
    await tx.execute(
      sql`select pg_advisory_xact_lock(hashtextextended(${`${actor}:subscription:${keyDigest}`}, 0))`,
    );
    const [existing] = await tx
      ...
      .where(
        and(
          ...
          eq(idempotencyRecords.operation, "subscription.create"),
    ...
    const [price] = await tx
      ...
      .innerJoin(
        subscriptionPlans,
        and(
          eq(subscriptionPlans.organizationId, recurringPrices.organizationId),
          eq(subscriptionPlans.id, recurringPrices.planId),
        ),
      )
      .where(
        and(
          eq(recurringPrices.organizationId, context.tenant.organizationId),
          eq(recurringPrices.id, input.price_id),
          eq(recurringPrices.status, "active"),
    ...
    const trialEnd = input.trial_days
      ? new Date(now.getTime() + input.trial_days * 86_400_000)
      : null;
    const period = initialBillingPeriod(
      trialEnd ?? now,
      recurringPrice.interval as BillingInterval,
    );
    const [row] = await tx
    ...
    await tx.insert(idempotencyRecords).values({
      ...
      requestDigest,
      responseStatus: 201,
      responseBody: { subscription: response },
    ...
    await recordDomainChange(tx, context, {
      ...
      data: { subscription_id: row.id, status: row.status, price_id: row.priceId },
    });
    if (trialEnd)
      await recordDomainChange(tx, context, {
        ...
        aggregateId: row.id,
        aggregateVersion: row.version,
        data: { subscription_id: row.id, trial_end: trialEnd.toISOString() },
      });
    return response;
  });
}
...
export async function listSubscriptions(
  ...
  return withTenantTransaction(context.tenant, async (tx) => {
    await requirePermission(tx, context.principal, "subscriptions:read", {
      organizationId: context.tenant.organizationId,
    });
    const predicates = [
