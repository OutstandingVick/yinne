...
async function mutate(
  ...
  id: string,
  permission: "subscriptions:write" | "subscriptions:cancel",
  handler: (row: typeof subscriptions.$inferSelect) => {
    status: string;
    action:
      | "subscription.paused"
  ...
  return withTenantTransaction(context.tenant, async (tx) => {
    ...
    const [updated] = await tx
      ...
      .set({ ...change.values, version: sql`${subscriptions.version} + 1`, updatedAt: new Date() })
    ...
    await recordDomainChange(tx, context, {
      ...
      data: { subscription_id: updated.id, status: change.status },
    });
...
export function pauseSubscription(context: RequestContext, id: string) {
  return mutate(context, id, "subscriptions:write", (row) => {
    ...
    return {
      ...
      values: { status: "paused", pausedAt: new Date(), nextBillingAt: null },
    };
  });
}
...
export async function retrySubscription(
  ...
  await withTenantTransaction(context.tenant, async (tx) => {
    await requirePermission(tx, context.principal, "subscriptions:retry", {
      organizationId: context.tenant.organizationId,
    });
    const [row] = await tx
      .select()
    ...
    if (!row)
      throw new ApiError(
        ...
        "Only a past-due Subscription may be retried.",
      );
    await tx
      .update(subscriptions)
      .set({
        nextBillingAt: now,
        ...(input.mock_outcome ? { mockRenewalOutcome: input.mock_outcome } : {}),
        updatedAt: now,
      })
      .where(eq(subscriptions.id, id));
    await recordDomainChange(tx, context, {
      action: "subscription.renewal_retried",
      aggregateType: "subscription",
      aggregateId: row.id,
      aggregateVersion: row.version,
      data: { subscription_id: row.id, retry_count: row.retryCount + 1 },
    });
  });
