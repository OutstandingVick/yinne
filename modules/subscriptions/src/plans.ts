...
export async function createPlan(context: RequestContext, input: CreateSubscriptionPlanInput) {
  return withTenantTransaction(context.tenant, async (tx) => {
    ...
    const [row] = await tx
      ...
      .values({ organizationId: context.tenant.organizationId, ...input })
      .returning();
    if (!row)
      throw new ApiError(500, "internal_error", "plan_create_failed", "Plan could not be created.");
    await recordDomainChange(tx, context, {
      action: "subscription_plan.created",
      aggregateType: "subscription_plan",
      aggregateId: row.id,
...
export async function getPlan(context: RequestContext, id: string) {
  return withTenantTransaction(context.tenant, async (tx) => {
    ...
    const [row] = await tx
      ...
      .where(
        and(
          eq(subscriptionPlans.organizationId, context.tenant.organizationId),
    ...
    const prices = await tx
      ...
      .where(
        and(
          ...
          eq(recurringPrices.planId, id),
        ),
      )
      .orderBy(desc(recurringPrices.createdAt));
  ...
  return withTenantTransaction(context.tenant, async (tx) => {
    ...
    const [plan] = await tx
      ...
      .where(
        and(
          eq(subscriptionPlans.organizationId, context.tenant.organizationId),
          eq(subscriptionPlans.id, input.plan_id),
          eq(subscriptionPlans.status, "active"),
        ),
      )
      .limit(1);
    ...
    const [row] = await tx
      ...
      .values({
        ...
        unitAmount: BigInt(input.unit_amount),
    ...
    if (!row)
      throw new ApiError(
        ...
        "internal_error",
        "price_create_failed",
        "Price could not be created.",
      );
    await recordDomainChange(tx, context, {
      action: "recurring_price.created",
      aggregateType: "recurring_price",
      aggregateId: row.id,
      aggregateVersion: 1,
      data: { price_id: row.id, plan_id: row.planId },
    });
...
export async function archivePrice(context: RequestContext, id: string) {
  return withTenantTransaction(context.tenant, async (tx) => {
    ...
    const [row] = await tx
      .update(recurringPrices)
      .set({ status: "archived", archivedAt: new Date() })
      .where(
        and(
          eq(recurringPrices.organizationId, context.tenant.organizationId),
          eq(recurringPrices.id, id),
    ...
    if (!row) notFound();
    await recordDomainChange(tx, context, {
      action: "recurring_price.archived",
      aggregateType: "recurring_price",
      aggregateId: row.id,
      aggregateVersion: 1,
      data: { price_id: row.id },
    });
    return priceView(row);
  });
}
