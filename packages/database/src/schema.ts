...
export const marketplaceProfiles = pgTable(
  ...
  (table) => [
    foreignKey({
      name: "marketplace_profiles_merchant_org_fk",
      columns: [table.organizationId, table.merchantId],
      foreignColumns: [merchants.organizationId, merchants.id],
    }),
    uniqueIndex("marketplace_profiles_market_org_env_uidx").on(
      table.marketplaceId,
      table.organizationId,
      table.environment,
    ),
    uniqueIndex("marketplace_profiles_market_env_slug_uidx").on(
      table.marketplaceId,
      table.environment,
      table.slug,
    ),
...
export const marketplaceListings = pgTable(
  ...
  (table) => [
    foreignKey({
      name: "marketplace_listings_profile_org_fk",
      columns: [table.organizationId, table.profileId],
      foreignColumns: [marketplaceProfiles.organizationId, marketplaceProfiles.id],
    }),
    foreignKey({
      name: "marketplace_listings_product_org_fk",
      columns: [table.organizationId, table.productId],
      foreignColumns: [products.organizationId, products.id],
    }),
    uniqueIndex("marketplace_listings_org_id_uidx").on(table.organizationId, table.id),
    uniqueIndex("marketplace_listings_market_product_active_uidx")
      .on(table.marketplaceId, table.environment, table.productId)
      .where(sql`${table.status} <> 'archived'`),
    index("marketplace_listings_public_idx").on(
      table.marketplaceId,
      table.environment,
      table.status,
      table.categoryId,
    ...
    check(
      ...
      sql`${table.status} in ('draft', 'submitted', 'approved', 'rejected', 'suspended', 'archived')`,
    ),
  ],
);
