...
export const marketplaceSlugSchema = z.string().trim().regex(/^[a-z0-9]+(?:-[a-z0-9]+)*$/).max(80);
export const marketplaceListingStatusSchema = z.enum([
  "draft", "submitted", "approved", "rejected", "suspended", "archived",
]);
...
export const marketplaceModerationSchema = z.object({
  reason_code: z.string().trim().regex(/^[a-z0-9_]+$/).max(80),
...
}).refine((v) => v.min_amount === undefined || v.max_amount === undefined || v.min_amount <= v.max_amount, "Invalid price range.");
